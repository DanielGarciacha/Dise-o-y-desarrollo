...
def login():
    try:
        ...
        cur = mysql.connection.cursor()
        # Cambia 'usuarios' por 'user'
        query = 'SELECT username, password, rol FROM user WHERE username = %s AND password = %s'
        ...
        if user:
            ...
            return jsonify({
                'success': True,
                'token': access_token,  # Asegúrate de que esto coincida con lo que espera tu JS
                'username': username,
                'role': user[2]
            })
        else:
